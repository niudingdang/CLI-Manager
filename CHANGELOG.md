...
- **原文对话模式**：历史会话「原文」页签改为聊天气泡展示，通过方形圆角用户/AI 头像和不同气泡配色区分消息来源；长消息默认省略预览，可展开查看全文，并使用当前主题变量适配浅色/深色主题。
...
- **Trellis 启动版号确认**：本地 Trellis 工作流现在要求在进入会写文件的任务前，先确认本次变更应归入 `CHANGELOG.md` 的哪个版本；如果用户没有提供版本号，则统一记入临时版本 `[TEMP]`，不再猜测正式发布版本。
